...
def plot_low_soc_timeseries(interval_data, interval_time_steps, threshold=10):
    plt.figure(figsize=(12, 6))
    for vehicle_id, soc_values in interval_data.items():
        if any(soc < threshold for soc in soc_values if not np.isnan(soc)):
            plt.plot(interval_time_steps, soc_values, label=vehicle_id)
...
def plot_statistics(data, time_steps, interval=300, data_type='SOC'):
...
def main(file_path):
    ...
    #plot_low_soc_timeseries(interval_data, interval_time_steps, threshold=10)
    plot_statistics(soc_data_aligned, time_steps, interval=300, data_type='SOC')
    plot_statistics(speed_data_aligned, time_steps, interval=300, data_type='Speed')
